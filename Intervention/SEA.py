...
def fit_or_load_probe_ce(
    ...
    seed: int = 0,
    input_mode: str = "raw",      # 新增: "raw" = 不带模板
    ...
    probe_path = os.path.join(save_dir, f"probe_{tag}_{input_mode}_L{probe_layer_idx}.pt")
    ...
    X_pos = extract_feature_matrix_for_texts_cached(
        model, processor, system_prompt, texts_pos, probe_layer_idx, save_dir, tag, split_name="train_pos", input_mode=input_mode, raw_max_len=raw_max_len
    )
    X_neg = extract_feature_matrix_for_texts_cached(
        model, processor, system_prompt, texts_neg, probe_layer_idx, save_dir, tag, split_name="train_neg", input_mode=input_mode, raw_max_len=raw_max_len
    )
...
def extract_feature_matrix_for_texts_cached(
    ...
    input_mode: str = "raw",        # "chat"（原逻辑）或 "raw"（论文式，不带模板）
    raw_max_len: int = 1024,
) -> torch.Tensor:
    """
    ...
    """
    os.makedirs(save_dir, exist_ok=True)
    cache_fp = os.path.join(save_dir, f"feats_{tag}_{split_name}_L{layer_idx}_{input_mode}.npy")
    ...
    for t in texts:
        if input_mode == "raw":
            # 论文做法：喂原始句子，不加 chat 模板
            v = extract_feature_mean_hidden_at_layer_raw(
                model, processor, t, layer_idx=layer_idx, max_tokens=raw_max_len
            )
        else:
            # 原有做法：chat 模板 + 你的评分提示包装
            v = extract_feature_mean_hidden_at_layer(
                model, processor, system_prompt, build_user_prompt(t), layer_idx=layer_idx
            )
        feats.append(v)
...
def run_model_surgery_once(
    ...
    probe_layer_idx: int = 28,     # 你也可以试 -2 / 31 等（论文在多个层试过）
    ...
    X_pos_tr = extract_feature_matrix_for_texts_cached(
        model, processor, SYSTEM_PROMPT, pos_tr, probe_layer_idx, "probe_ckpts", "stance_invariance", "train_pos", input_mode=("raw" if use_raw_text_for_probe else "chat"),
        raw_max_len=raw_max_len_for_probe
    )
    ...
    X_neg_te = extract_feature_matrix_for_texts_cached(
        model, processor, SYSTEM_PROMPT, neg_te, probe_layer_idx, "probe_ckpts", "stance_invariance", "test_neg", input_mode=("raw" if use_raw_text_for_probe else "chat"),
        raw_max_len=raw_max_len_for_probe
    )
...
# ===========================
# SEA: Spectral Editing of Activations
# ===========================
# 超参：子空间阈值与启用的末层数
SEA_VARIANCE_TAU = 0.90         # 累计奇异值比例阈值 τ（论文用 explained variance ratio 选 k）[可调]
SEA_USE_LAST_LAYERS = 13      # None=全部层；或设成比如 16 只用最后16层
SEA_MIX_LAMBDA = 1.0            # λ=1 完全替换；(0,1) 混合更保守

@torch.no_grad()
def _append_token(input_ids: torch.Tensor, attention_mask: torch.Tensor, token_id: int):
    """把一个 token 直接接到输入末尾（teacher-forcing），返回新的 (ids, mask, new_answer_pos)."""
    bsz = input_ids.size(0)
    dev = input_ids.device
    tok = torch.tensor([[token_id]], device=dev)
    new_ids = torch.cat([input_ids, tok], dim=1)
    if attention_mask is None:
        new_mask = None
    else:
        tokm = torch.ones((bsz,1), dtype=attention_mask.dtype, device=dev)
        new_mask = torch.cat([attention_mask, tokm], dim=1)
    new_pos = new_ids.shape[1] - 1
    return new_ids, new_mask, new_pos

def _predict_digit_id_for_text(
    processor: AutoProcessor, model: Gemma3ForConditionalGeneration, system_prompt: str, statement: str
) -> int:
    """对单个文本跑一遍，取 {1..7} 上最大概率的数字 token id。"""
    enc = encode_for_next_token(processor, model, system_prompt, build_user_prompt(statement))
    with torch.no_grad():
        logits = forward_logits_only(model, enc)
    digits = digit_logit_slice(logits, enc.digit_ids)
    return enc.digit_ids[int(torch.argmax(digits).item())]

@torch.no_grad()
def _gather_mlp_answer_activations_for_texts(
    model: Gemma3ForConditionalGeneration,
    processor: AutoProcessor,
    system_prompt: str,
    texts: List[str],
    kind: str,  # "neutral" / "pos" / "neg"
    pos_pool=(5,6,7),
    neg_pool=(1,2,3),
    max_samples: Optional[int] = 512,
) -> Dict[int, torch.Tensor]:
    """
    收集每层 Gemma3MLP 在“答案位点”的输出激活。
    - neutral: 只编码提示（不追加答案 token）
    - pos/neg: 用 teacher-forcing 把“正/负方向”的评分数字 token 追加到末尾，激活取追加后的最后一位。
      具体要追加哪个数字：我们先让模型对该句做一次前向，取 {pos_pool/neg_pool} 中“模型偏好的那个”，使 H⁺/H⁻ 贴合模型自身分布。
    返回: layer_idx -> [N,H] 的张量（按顺序堆叠）
    """
    layers = get_decoder_layers(model)
    n_layers = len(layers)
    use_layers = list(range(n_layers)) if SEA_USE_LAST_LAYERS is None else list(range(max(0, n_layers-SEA_USE_LAST_LAYERS), n_layers))
    dev = get_input_device(model)

    feats: DefaultDict[int, List[torch.Tensor]] = defaultdict(list)
    # 预挂钩子：抓每层 MLP 输出
    hooks = []
    last_pos_holder = {"pos": 0}  # 动态更新当前样本的答案位点

    def make_hook(layer_idx):
        def _hook(mod, inp, out):
            # Gemma3MLP forward 输出形状和 block 一致（[B, seq, H]）
            hidden = out[0] if isinstance(out, tuple) else out
            pos = last_pos_holder["pos"]
            feats[layer_idx].append(hidden[:, pos, :].detach().squeeze(0).to("cpu", torch.float32))
            return out
        return _hook

    for i, name, block in layers:
        for subname, sub in block.named_modules():
            if isinstance(sub, Gemma3MLP) and i in use_layers:
                hooks.append(sub.register_forward_hook(make_hook(i)))

    # 逐条样本前向，收集激活
    num = len(texts) if max_samples is None else min(max_samples, len(texts))
    for t in texts[:num]:
        enc = encode_for_next_token(processor, model, system_prompt, build_user_prompt(t))
        ids, mask = enc.input_ids, enc.attention_mask

        if kind == "neutral":
            # 答案位点即当前末位
            last_pos_holder["pos"] = ids.shape[1]-1
            _ = model(input_ids=ids, attention_mask=mask, output_hidden_states=False, return_dict=True)
        else:
            with torch.no_grad():
                base_logits = forward_logits_only(model, enc)
            digits = digit_logit_slice(base_logits, enc.digit_ids)  # [7]
            # 只在 pos_pool/neg_pool 中挑 argmax
            pool = pos_pool if kind == "pos" else neg_pool
            pool_ids = [enc.digit_ids[d-1] for d in pool]
            pool_logits = base_logits.index_select(dim=-1, index=torch.tensor(pool_ids, device=base_logits.device))
            top_idx_in_pool = int(torch.argmax(pool_logits).item())
            chosen_token_id = pool_ids[top_idx_in_pool]

            ids2, mask2, ans_pos2 = _append_token(ids, mask, chosen_token_id)
            last_pos_holder["pos"] = ans_pos2
            _ = model(input_ids=ids2, attention_mask=mask2, output_hidden_states=False, return_dict=True)
    ...
    for l in use_layers:
        if l in feats and len(feats[l]) > 0:
            out[l] = torch.stack(feats[l], dim=0).contiguous()
    return out

def _cross_cov(A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
    """
    经验交叉协方差: A, B 形状 [N,H]，先中心化再乘：
      C = (A - Ā)^T (B - \bar B) / (N - 1)
    """
    A = A.float(); B = B.float()
...
def _pick_k_by_variance(svals: torch.Tensor, tau: float, smallest: bool=False) -> int:
    """
    按累计奇异值比例阈值 tau 选 k。
    - 对 C_{+n}：保留“最大”奇异值对应的左奇异向量 → smallest=False
    - 对 C_{-n}：保留“最小”奇异值对应的左奇异向量 → smallest=True
    """
    s = torch.clamp(svals.float(), min=0)
    if smallest:
        s = torch.flip(s, dims=[0])
    cumsum = torch.cumsum(s, dim=0)
    total = s.sum() + 1e-12
    frac = cumsum / total
    k = int(torch.searchsorted(frac, torch.tensor(tau, dtype=frac.dtype)) + 1)
    k = max(1, min(k, svals.numel()))
    return k
...
def sea_fit(
    ...
    texts_pos: List[str],
    texts_neg: List[str],
    max_samples_per_set: int = 512,
    tau: float = SEA_VARIANCE_TAU,
):
    """
    拟合 SEA：返回 per-layer 的 (P_plus, P_minus) 投影矩阵字典。
    - Hⁿ（neutral）来自 pos+neg 的提示集合（不追加答案）；
    - H⁺ 来自 pos 集合上 teacher-forced 的“正向评分”答案位点激活；
    - H⁻ 来自 neg 集合上 teacher-forced 的“负向评分”答案位点激活。
    """
    # 收集激活
    Hn_pos = _gather_mlp_answer_activations_for_texts(model, processor, system_prompt, texts_pos, kind="neutral",  max_samples=max_samples_per_set)
    Hn_neg = _gather_mlp_answer_activations_for_texts(model, processor, system_prompt, texts_neg, kind="neutral",  max_samples=max_samples_per_set)
    # 合并两份中性，样本更多更稳
    Hn = {}
    for l in set(Hn_pos.keys()) | set(Hn_neg.keys()):
        xs = []
        if l in Hn_pos: xs.append(Hn_pos[l])
        if l in Hn_neg: xs.append(Hn_neg[l])
        Hn[l] = torch.cat(xs, dim=0)

    Hplus = _gather_mlp_answer_activations_for_texts(model, processor, system_prompt, texts_pos, kind="pos", max_samples=max_samples_per_set)
    Hminus= _gather_mlp_answer_activations_for_texts(model, processor, system_prompt, texts_neg, kind="neg", max_samples=max_samples_per_set)

    layers = get_decoder_layers(model)
    n_layers = len(layers)
    use_layers = list(range(n_layers)) if SEA_USE_LAST_LAYERS is None else list(range(max(0, n_layers-SEA_USE_LAST_LAYERS), n_layers))

    P_plus: Dict[int, torch.Tensor] = {}
    P_minus: Dict[int, torch.Tensor] = {}

    for l in use_layers:
        if (l not in Hn) or (l not in Hplus) or (l not in Hminus): 
            continue
        A = Hplus[l]   # [N,H]
        B = Hn[l]      # [M,H]  — 为了构造 C_{+n}，简单地对齐到相同样本数：随机子采样到 min(N,M)
        C = Hminus[l]  # [K,H]

        # 对齐样本数
        def _match(X, target):
            if X.shape[0] <= target: return X
            idx = torch.randperm(X.shape[0])[:target]
            return X.index_select(0, idx)
        m = min(A.shape[0], B.shape[0])
        A_ = _match(A, m); B_ = _match(B, m)
        m2= min(C.shape[0], B.shape[0])
        C_ = _match(C, m2); B2= _match(B, m2)

        # C_{+n} 与 C_{-n}
        C_plusn  = _cross_cov(A_, B_)     # [H,H]
        C_minusn = _cross_cov(C_, B2)     # [H,H]

        # SVD
        # torch.linalg.svd 返回 U, S, Vh，其中 U 为左奇异向量（我们要的子空间基）
        U_p, S_p, Vh_p = torch.linalg.svd(C_plusn, full_matrices=False)
        U_m, S_m, Vh_m = torch.linalg.svd(C_minusn, full_matrices=False)

        kp = _pick_k_by_variance(S_p, tau=tau, smallest=False)   # 最大奇异向量
        km = _pick_k_by_variance(S_m, tau=tau, smallest=True)    # 最小奇异向量

        Up = U_p[:, :kp]               # [H,kp]
        # 负类：保留“最小奇异值方向”的子空间（论文描述）
        Um_small = U_m[:, -km:]        # 注意：SVD 默认降序，这里取“尾部” km 个最小奇异值方向  → [H,km]

        # 投影矩阵
        P_plus[l]  = Up @ Up.t()                       # onto positive-cov subspace
        P_minus[l] = Um_small @ Um_small.t()           # onto least-negative-cov subspace

    return P_plus, P_minus

@contextlib.contextmanager
def sea_edit_context(
    model: Gemma3ForConditionalGeneration,
    enc: EncodedChat,
    P_plus: Dict[int, torch.Tensor],
    P_minus: Dict[int, torch.Tensor],
    mix_lambda: float = SEA_MIX_LAMBDA,
):
    """
    在答案位点对每层 Gemma3MLP 输出做 SEA 成对投影：
      h -> h_plus = P⁺ h; h -> h_minus = P⁻ h;  \tilde h = P⁻(P⁺ h)
      h_new = (1-λ) h + λ \tilde h
    重要：P 矩阵在 hook 内对齐到 hidden 的 device 和 dtype（解决 bf16/多 GPU 分片问题）。
    """
    hooks = []
    layers = get_decoder_layers(model)
    use_layers = set(P_plus.keys()) & set(P_minus.keys())
    ans_pos = enc.answer_pos

    def make_hook(layer_idx):
        Pp_src = P_plus[layer_idx]    # 通常是 float32, CPU/GPU 都可能
        Pm_src = P_minus[layer_idx]

        # 为了减少重复 to()，做个小缓存（当 hidden 的 device/dtype 变化时再搬运/转型）
        cache = {"key": None, "Pp": None, "Pm": None}

        def _hook(mod, inp, out):
            hidden = out[0] if isinstance(out, tuple) else out
            # 取当前这层真正的 device + dtype
            d  = hidden.device
            dt = hidden.dtype
            key = (d, dt)
            if cache["key"] != key:
                cache["Pp"] = Pp_src.to(device=d, dtype=dt, non_blocking=True).contiguous()
                cache["Pm"] = Pm_src.to(device=d, dtype=dt, non_blocking=True).contiguous()
                cache["key"] = key
            Pp = cache["Pp"]
            Pm = cache["Pm"]

            # 保险：答案位点不要越界（虽然通常不会）
            pos = ans_pos if ans_pos < hidden.size(1) else (hidden.size(1) - 1)

            h = hidden[:, pos, :]                      # [B, H]
            # 注意：P 是对称投影，P^T == P，但写 .t() 无妨
            h_plus  = torch.matmul(h, Pp.t())
            h_tilde = torch.matmul(h_plus, Pm.t())
            h_new   = (1.0 - mix_lambda) * h + mix_lambda * h_tilde

            new_hidden = hidden.clone()
            new_hidden[:, pos, :] = h_new
            return (new_hidden, *out[1:]) if isinstance(out, tuple) else new_hidden
...
@torch.no_grad()
def _gather_mlp_answer_activations_simple(
    model: Gemma3ForConditionalGeneration,
    processor: AutoProcessor,
    system_prompt: str,
    texts: List[str],
    ...
    """
    对一组文本（如 base 或 variant），在“预测分数之前”的答案位点收集每层 Gemma3MLP 的输出激活。
    不追加任何答案数字；与评测时的真实前向一致。
    返回: layer_idx -> [N, H]
    """
    from collections import defaultdict
    ...
    def make_hook(lidx):
        def _hook(mod, inp, out):
            hidden = out[0] if isinstance(out, tuple) else out
            feats[lidx].append(hidden[:, pos_holder["pos"], :].detach().squeeze(0).to("cpu", torch.float32))
            return out
        return _hook

    for i, _, block in layers:
        if i in use_layers:
            for subname, sub in block.named_modules():
                if isinstance(sub, Gemma3MLP):
                    hooks.append(sub.register_forward_hook(make_hook(i)))
    ...
    for t in texts[:num]:
        enc = encode_for_next_token(processor, model, system_prompt, build_user_prompt(t))
        pos_holder["pos"] = enc.answer_pos
        _ = model(input_ids=enc.input_ids, attention_mask=enc.attention_mask, output_hidden_states=False, return_dict=True)
...
def sea_fit_pairs(
    ...
    """
    H⁺ := base 的答案位点激活；H⁻ := variant 的答案位点激活；
    Hⁿ := 简便做法为 H⁺ ∪ H⁻ 的合并（做中心化后与二者分别做交叉协方差）。
    返回: per-layer 的 (P_plus, P_minus)
    """
    Hplus  = _gather_mlp_answer_activations_simple(model, processor, system_prompt, texts_base,    max_samples=max_samples_per_set)
    Hminus = _gather_mlp_answer_activations_simple(model, processor, system_prompt, texts_variant, max_samples=max_samples_per_set)

    layers_all = sorted(set(Hplus.keys()) & set(Hminus.keys()))
    P_plus: Dict[int, torch.Tensor]  = {}
    P_minus: Dict[int, torch.Tensor] = {}
    ...
    for l in layers_all:
        A = Hplus[l]          # [N+, H]
        C = Hminus[l]         # [N-, H]
        B = torch.cat([A, C], dim=0)  # 作为“中性参考” Hⁿ
        ...
        m1 = min(A.shape[0], B.shape[0]); A1 = _match(A, m1); B1 = _match(B, m1)
        m2 = min(C.shape[0], B.shape[0]); C1 = _match(C, m2); B2 = _match(B, m2)

        # C_{+n}, C_{-n}
        C_plusn  = _cross_cov(A1, B1)   # [H,H]
        C_minusn = _cross_cov(C1, B2)   # [H,H]

        # SVD
        U_p, S_p, _ = torch.linalg.svd(C_plusn,  full_matrices=False)
        U_m, S_m, _ = torch.linalg.svd(C_minusn, full_matrices=False)

        # 选子空间维数：对 C_{+n} 取“最大奇异值方向”累计阈值；对 C_{-n} 取“最小奇异值方向”累计阈值
        kp = _pick_k_by_variance(S_p, tau=tau, smallest=False)
        km = _pick_k_by_variance(S_m, tau=tau, smallest=True)

        Up = U_p[:, :kp]           # [H, kp]
        Um_small = U_m[:, -km:]    # [H, km]  —— 取“尾部”的最小奇异值方向

        P_plus[l]  = Up @ Up.t()
        P_minus[l] = Um_small @ Um_small.t()
...
def run_SEA_once(
    ...
    # === SEA 拟合：用训练集合的一部分构建 P⁺/P⁻ ===
    # 这里直接把 base_texts 当作“正类提示集合”，variant_texts 当作“负类提示集合”；
    # Hⁿ 来自两边提示（不加答案），H⁺ 来自 base 上“正向评分” teacher-forcing，H⁻ 来自 variant 上“负向评分” teacher-forcing。
    print("[SEA] Fitting spectral projectors...")
    P_plus, P_minus = sea_fit_pairs(
        model, processor, SYSTEM_PROMPT,
        texts_base=base_texts,
        texts_variant=variant_texts,
        max_samples_per_set=512,
        tau=SEA_VARIANCE_TAU
    )

    used_layers = sorted(set(P_plus.keys()) & set(P_minus.keys()))
    print(f"[SEA] Fitted layers: {used_layers[:10]}{'...' if len(used_layers)>10 else ''} (count={len(used_layers)})")

    # === 通用能力基线（未编辑）
    base_nll, base_ppl, base_tok = evaluate_wikitext_ppl(
        model, processor, dataset_config="wikitext-2-raw-v1",
        split="test", block_size=None, stride=None, max_texts=200
    )
    print(f"[WikiText] baseline: NLL={base_nll:.4f}, PPL={base_ppl:.3f}, toks={base_tok}")

    # === 评测 clean / corrupt / patched 分布
    enc_clean   = encode_for_next_token(processor, model, SYSTEM_PROMPT, build_user_prompt(eval_pair[0]))
    enc_corrupt = encode_for_next_token(processor, model, SYSTEM_PROMPT, build_user_prompt(eval_pair[1]))
    with torch.no_grad():
        logits_clean   = forward_logits_only(model, enc_clean)
    ...
    # 应用 SEA（λ 可调；默认 1.0）
    with sea_edit_context(model, enc_corrupt, P_plus, P_minus, mix_lambda=SEA_MIX_LAMBDA):
        logits_patched = forward_logits_only(model, enc_corrupt)
        patched_probs  = digit_probs_from_logits_full(logits_patched, enc_clean, TEMP_FOR_PROBS)
        # 通用能力在编辑开启下的 PPL
        patched_nll, patched_ppl, _ = evaluate_wikitext_ppl(
            model, processor, dataset_config="wikitext-2-raw-v1", split="test",
            block_size=None, stride=None, max_texts=200
        )

    r_w1 = normalized_restoration(w_1d,         clean_probs, corrupt_probs, patched_probs).item()

    print(f"[SEA] clean_probs  = {clean_probs}")
    print(f"[SEA] corrupt_probs= {corrupt_probs}")
    print(f"[SEA] patched_probs= {patched_probs}")
    print(f"[SEA] R_W1={r_w1:.3f}")
    if base_ppl == base_ppl and patched_ppl == patched_ppl:
        delta_pct = (patched_ppl / base_ppl - 1.0) * 100.0
        print(f"[WikiText] baseline PPL={base_ppl:.3f} → SEA PPL={patched_ppl:.3f} (Δ={delta_pct:.1f}%)")
...
if __name__ == "__main__":
    ...
    run_SEA_once(train_base, train_variant, eval_pair)
