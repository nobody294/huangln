...
def restoration_toward_clean(p_clean: torch.Tensor, p_corrupt: torch.Tensor, p_new: torch.Tensor, eps: float = 1e-12) -> float:
    """
    计算 R = 1 - d(clean,new)/d(clean,corrupt)
    （这是“回到 clean”的 restoration，适用于 unflip 统计）
    """
    p_target = flip_probs_1_to_7(p_clean)
    d0 = w_1d(p_target, p_corrupt)
    dp = w_1d(p_target, p_new)
    R = 1.0 - dp / (d0 + eps)
    if float(d0.item()) <= eps:
        return float("nan")
    return float(R.item())


def count_unflips_block_ablation_layer(
    model,
    tokenizer,
    pairs: List[Tuple[str, str]],
    layer_idx: int = 34,
    ...
    """
    统计：在 variant 上 ablate block 的第 layer_idx 层（ratio=0.0 默认全抹掉 answer_pos 向量），
    有多少 flipped pair 被“unflip”回 clean 极性。
    """
    n_layers = len(get_decoder_layers(model))
    if layer_idx < 0 or layer_idx >= n_layers:
        raise ValueError(f"layer_idx={layer_idx} out of range (n_layers={n_layers}). 先检查模型层数。")
    ...
    for i, (b, v) in enumerate(pairs, 1):
        if verbose and (i % 5 == 1 or i == len(pairs)):
            print(f"[block ablate L{layer_idx}] pair {i}/{len(pairs)}")
        ...
        # ablate block layer_idx on corrupt input
        with block_ablation_context(model, enc_corrupt, [layer_idx], ratio=ratio):
            logits_ab = forward_logits_only(model, enc_corrupt)
            ab_probs_t = digit_probs_from_logits_full(logits_ab, enc_corrupt, TEMP_FOR_PROBS)

        ab_probs = ab_probs_t.detach().float().cpu().numpy()
        pol_ab = polarity_from_probs(ab_probs, tie_eps=tie_eps)

        R_clean = restoration_toward_clean(clean_probs_t, corrupt_probs_t, ab_probs_t, eps=EPS)

        unflipped = flipped and (pol_ab != pol_clean) and (not np.isnan(R_clean)) and (R_clean > restoration_thresh)
        if unflipped:
            unflip_count += 1
...
def main():
    ...
    unflip34, flipped34, details34 = count_unflips_block_ablation_layer(
        model=model,
        tokenizer=tokenizer,
        pairs=PAIRS,
        layer_idx=34,
        ratio=0.0,
        restoration_thresh=0.0,  # 想严格点可改 0.3 / 0.5
        tie_eps=0.1,
        verbose=True,
    )

    print("\n================ UNFLIP COUNT (ABLATION) ================")
    print(f"Block ablation layer 34 (ratio=0.0): unflip {unflip34} / flipped-eligible {flipped34} (total pairs={len(PAIRS)})")

    # 可选：导出每对句子的判定细节
    import csv
    with open("unflip_block_ablate_L34_details.csv", "w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=list(details34[0].keys()))
        w.writeheader()
        w.writerows(details34)
    print("Saved: unflip_block_ablate_L34_details.csv")
