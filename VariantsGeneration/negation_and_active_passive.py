...
output_csv_path    = "data/combine_negation_active_passive_variants.csv"

model_name = "Qwen/Qwen3-8B"

SYSTEM_PROMPT = (
    "You are a controlled text rewriter for logical variants of short English statements. "
    "You will be given a BASE statement, its NEGATION variant, and its ACTIVE/PASSIVE CONVERSION variant. "
...
BUILTIN_FEWSHOTS = [
    {
        ...
        "combine variant": "Stronger financial support should not be provided to unemployed workers by the state.",
    },
    {
        ...
        "combine variant": "Member States that violate the EU deficit rules should not be rigorously punished by the EU.",
    },
    {
        ...
        "combine variant": "The government should not tax bank and stock market gains more heavily.",
    },
    {
        ...
        "combine variant": "In European Parliament elections, a vote should not be allowed to be cast by EU citizens for a party or candidate from any other Member State.",
    },
    {
    ...
    },
    {
        ...
        "combine variant": "The government is not to welcome the legalisation of the personal use of soft drugs.",
    },
]
...
def build_user_prompt(base: str, neg: str, actpas: str, fewshots_text: str) -> str:
    schema = """{
    ...
    return f"""Task: Construct a COMBINE variant from a base, a negation variant, and an active/passive variant.
...
def run():
    ...
    for i, row in merged.iterrows():
        ...
        user_prompt = build_user_prompt(base_str, neg_str, actpas_str, fewshots_text)
