...
output_csv_path    = "data/combine_negation_SVC_variants.csv"

model_name = "Qwen/Qwen3-8B"

SYSTEM_PROMPT = (
    "You are a controlled text rewriter for logical variants of short English statements. "
    "You will be given a BASE statement, its NEGATION variant, and its SUPPORT VERB CONSTRUCTION variant. "
...
BUILTIN_FEWSHOTS = [
    {
        ...
        "combine variant": "The state should not make provision for stronger financial support to unemployed workers.",
    },
    {
        ...
        "combine variant": "The EU should not rigorously impose punishment on Member States that violate the EU deficit rules.",
    },
    {
        ...
        "combine variant": "Bank and stock market gains should not be given heavier taxation.",
    },
    {
        ...
        "combine variant": "In European Parliament elections, EU citizens should not be given permission to cast a vote for a party or candidate from any other Member State.",
    },
    {
...
def build_user_prompt(base: str, neg: str, svc: str, fewshots_text: str) -> str:
    schema = """{
    ...
    return f"""Task: Construct a COMBINE variant from a base, a negation variant, and a support verb construction variant.

    Hard constraints (follow strictly):
    1) Do NOT add new content, explanations, or paraphrases beyond support verb construction.
...
def run():
    ...
    for i, row in merged.iterrows():
        ...
        user_prompt = build_user_prompt(base_str, neg_str, svc_str, fewshots_text)
